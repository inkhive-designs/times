...
/* Sanitization Functions Common to Multiple Settings go Here, Specific Sanitization Functions are defined along with add_setting() */
	function times_sanitize_checkbox( $input ) {
	    if ( $input == 1 ) {
	        return 1;
	    } else {
	        return '';
	...
	function times_sanitize_positive_number( $input ) {
	...
	function times_sanitize_category( $input ) {
	...
	function times_sanitize_product_category( $input ) {
